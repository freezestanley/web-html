...
- 禁止read 反复验证文件完整性 — 规避策略：改用 exec：wc -l 查行数、tail -5 看结尾、grep 查关键字，禁止用 read 反复读完整内容
- subagent 完成后，主会话只保留最终结果摘要（3-5行），禁止把完整对话历史留在上下文中，禁止粘贴完整文件内容或代码
  - 粘贴 `dist/index.html` 全文
  - 粘贴大段 CSS / JS
  - 粘贴完整 diff
  - 粘贴浏览器截图数据
  - 输出超过 120 行的构建总结
...
## 限制规则:
  - 单轮构建🈯只1次桌面截图
  - 修复循环中禁止无变化重复截图
  - 截图前先用 console/network/DOM 检查排除明显错误
  - 截图结果只保留结论，不要围绕图片数据反复分析